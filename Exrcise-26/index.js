...
function createRockPlaylist(songsList) {
    timeReproduction = 5;
  for (const song of songsList) {
    if (song.genre === 'Rock' && song.duration > timeReproduction) {
      rockPlaylist.push({name: song.title,genre: song.genre ,duration: song.duration});
    }
  }
}
createRockPlaylist(songs);
